...
/* Five rungs of one ink at rising opacity, the same single-axis move the
   surface scale makes, so the graph reads as this site's material rather
   than as GitHub's green embedded in it. Level 0 is 6% rather than a flat
   `surface-sunken` fill so an empty day still sits *on* the canvas instead
   of punching a hole in it. */
const THEME = cn(
  'data-[level="0"]:fill-foreground/[0.06]',
  'data-[level="1"]:fill-foreground/20',
  'data-[level="2"]:fill-foreground/35',
  'data-[level="3"]:fill-foreground/55',
  'data-[level="4"]:fill-foreground/75',
);
...
export type ContributionGraphProps = HTMLAttributes<HTMLDivElement> & {
  ...
  children: ReactNode;
  className?: string;
  /* Target width for the calendar track. When set, `blockSize` is solved for
     rather than passed in, so the grid lands on exactly this width instead of
     near it. Derived from the real `weeks.length` (52, 53 or 54 depending on
     where the year boundary falls) so the fit holds through the turn of the
     year, which a hand-picked block size does not. */
  trackWidth?: number;
};
...
export const ContributionGraph = ({
  ...
  /* Inverse of the `width` line below: the track is n columns of
     (block + margin) with the last margin trimmed, so a target width solves
     to block = (width + margin) / n − margin. Left fractional on purpose,
     rounding to a whole pixel is what leaves the grid short of the column,
     and these are fills with no stroke, so a sub-pixel edge has nothing to
     blur. */
  const blockSize =
    trackWidth && weeks.length > 0
      ? Math.max(1, (trackWidth + blockMargin) / weeks.length - blockMargin)
      : blockSizeProp;
...
export const ContributionGraphCalendar = ({
  ...
  return (
    <div
      className={cn("no-scrollbar max-w-full overflow-x-auto overflow-y-hidden", className)}
      {...props}
    >
      <svg
